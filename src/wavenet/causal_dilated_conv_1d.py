...
class CausalDilatedConv1D(Conv1D):
    def __init__(self, nb_filter, filter_length,
        ...
        super(CausalDilatedConv1D, self).__init__(nb_filter, filter_length,
                                                  weights=weights,
        ...
        if self.causal and border_mode != 'valid':
            raise ValueError("border_mode=valid.")

    def compute_output_shape(self, input_shape):
        input_length = input_shape[1]
        if self.causal:
            input_length += self.atrous_rate * (self.filter_length - 1)
        length = conv_output_length(input_length, self.filter_length, self.border_mode, self.strides[0],
